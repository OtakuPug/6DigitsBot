	    					  ...
	    					  if(msg.getContentRaw().equalsIgnoreCase("heccyeah")) {
	    						  ...
	    						  if(msg.getContentRaw().equalsIgnoreCase("nuuu")) {
	    							  ...
	    							  if(msg.getContentRaw().equalsIgnoreCase("zakhere")) {
	    								  e.getMessage().delete().queue();
			    			    			 EmbedBuilder ebzakhere = new EmbedBuilder();
			    			    			 	ebzakhere.setColor(Color.green);
	    								  ebzakhere.setImage("https://cdn.discordapp.com/attachments/633814145854078988/636385466752565288/583008145337745418.gif"); 
	    								  channel.sendMessage(ebzakhere.build()).queue(); 
	    							  } else {
	    							  if(msg.getContentRaw().length() == 5 && letterTest) {
    									  channel.sendMessage("https://mangadex.org/title/" + msg.getContentRaw() + "/").queue(); //posts ad mangadex link from 5 digits
    								  		...
    								  		if (msg.getContentRaw().length() == 6 && letterTest && hkGuildTest && melsaGuildTest) {
    								  			  ...
    								  			  if(msg.getContentRaw().equalsIgnoreCase("gj")) {
    				    			    			 ...
    				    			    			 EmbedBuilder ebgj = new EmbedBuilder();
    								  				  ...
    								  				  if(msg.getContentRaw().equalsIgnoreCase("mhelp")) {
    				    			    			 	...
    				    			    			 	ebhelp.setThumbnail("https://cdn.discordapp.com/attachments/586990262170026001/637127429428609055/Melsa.png");
    				    			    			 	ebhelp.setDescription("do m<command> for help on that specific command\n\nType 6 digits for a nhentai link\nType 5 digits for a mangadex link\ngifs\nMisc.");
    				    			    			 	...
    				    			    			 	if(msg.getContentRaw().equalsIgnoreCase("mhelp gifs")) {
      				    			    			 	...
      				    			    			 	ebgifs.setThumbnail("https://cdn.discordapp.com/attachments/586990262170026001/637127429428609055/Melsa.png");
      				    			    			 	ebgifs.setDescription("gj\nzakhere\nnuuu\nbruh\nheccyeah");
      				    			    			 	channel.sendMessage(ebgifs.build()).queue(); 	
    								  				  } else {
    								  					  if(msg.getContentRaw().equalsIgnoreCase("mdelete")) {
    								  						  		e.getMessage().delete().queue();
